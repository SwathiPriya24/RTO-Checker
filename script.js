...
let findData = function () {
    let iputVal = ip.value.toLowerCase();
    console.log(iputVal)
    var foundData = ''
    vehicleInformation.filter(item => {
        let res = (item.district.toLowerCase().includes(iputVal)) || item.code.toLowerCase().includes(iputVal)
        ...
        if (res) {
            ...
            <ul> 
                ...
                <li><b>Location</b> : ${item.location}</li> 
                  <li><b>District</b> : ${item.district}</li>
            </div>`

       }
        container.innerHTML = foundData;
